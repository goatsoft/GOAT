...
The [Finder installer theme](../apps/goat-macos/art/dmg/README.md) uses the supplied artwork at 720 x 480 points. Packaging needs a logged-in Finder session with Automation permission. The DMG includes `CLI Tools/goat` and a `Licence` folder, and verifies the saved window and background after read-only remount. The neutral artwork capsule is decorative; the volume title records the validated version, channel and build. Existing published releases keep their original contents.
