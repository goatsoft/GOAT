...
`apps/goat-macos/release.json` is the editable product identity: canonical `version`, `codename` and positive integer `build`. The current development identity is **0.1.0 / Kid / 1340**, above the legacy build 1337. Increment the build when preparing a new candidate with changed content. The record alone does not make a candidate distributable.
...
| Secondary UI diagnostics | `Development · build 1340` or `Candidate · build 1340` |
