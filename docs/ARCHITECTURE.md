...
3. Shepherd captures a Sendable snapshot. Its worker reads attachment bytes and prepares the prompt off MainActor. The system turn is a pure function of stable chat state, so its bytes are identical across a turn's rounds and the engine prefix cache holds; per-round steering rides on the newest exchange as a `[GOAT note]` ([ADR-0085](adrs/0085-prefix-stable-prompts-and-usage-calibrated-budgeting.md)). `PromptBudgeter` selects whole context entries under deterministic limits and scales its estimate by a per-chat calibration ratio learned from server usage. Provider content is data, not authorization.
4. Inference normalizes the configured engine's HTTP/SSE response into typed generation events, including exact `usage` and any reported cached-prefix tokens. JUDAS admits the connection and rejects redirects. Display publications are coalesced; persistence checkpoints remain about one second apart.
...
Prompt-budget policy v4 charges protocol text (tool results, arguments, schemas, identifiers) near four bytes per token, calibrates against server usage per chat, reserves output by effort with a larger ceiling for reasoning-capable models, and falls back to a 16,384-token window when the engine reports none ([ADR-0085](adrs/0085-prefix-stable-prompts-and-usage-calibrated-budgeting.md)). It can replace older completed tool groups with labelled data excerpts while preserving the latest two groups exactly. Stored transcript data stays unchanged. Bleet keeps assistant reasoning and narration visible in stable rows inside its 40-message measured window. Adjacent rounds share alignment; only tool payloads are disclosed. One live status row reports observed activity, elapsed time and the last completed action ([ADR-0074](adrs/0074-grouped-transcript-tool-activity.md)). Live throughput includes coalesced generated tool-input bytes; waiting, generation and tool execution have distinct display states. See [ADRs 0069–0074](adrs/README.md).
