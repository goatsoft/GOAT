...
- Transcript max measure 68ch of the chat font (1ch is the advance of "0"), so it follows the chosen font and size. The transcript and composer share one column centered in wide windows; code blocks, tables and artifacts may extend past the measure to the code width token (820 pt). User bubbles ≤ 75% of the column, right-aligned; assistant messages full-measure with a 44-point light/dark masthead. GOAT 1337 substitutes its 60-point goat set.
...
- Assistant narration and reasoning remain visible when tool calls arrive. Stable message rows share alignment across consecutive assistant rounds. Only individual tool arguments/results are collapsed; failures, denials and missing results remain visible. Reasoning starts open with a bounded recent excerpt and explicit full-history/collapse controls. One live status row reports observed activity, exact elapsed time and the last completed action. Silence is not labelled prefill without engine evidence (amended ADR-0074).
