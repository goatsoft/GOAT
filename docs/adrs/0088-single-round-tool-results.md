...
`pen_run_command` runs the command and returns in one result: exit code, whether it timed out, and combined output bounded to 32 KiB with the middle elided and a marker giving the omitted size. `timeout_seconds` keeps its 1 to 600 range and the owner's default. When output exceeds the bound, the head and tail are kept with a marker giving the omitted byte count; GOAT writes no overflow file into the workspace, and the model redirects to its own workspace file (for example `command > out.log 2>&1`) and reads that with `pen_read_file` for the full log (owner decision 2026-09-12: no scratch spill, to keep the workspace clean). A `background: true` argument keeps today's job semantics for servers and watchers; only then is a job ID returned and `pen_command_status` and `pen_stop_command` remain for those jobs. Approval, seatbelt, whitelist, network policy and turn-end cleanup are unchanged. The command guidance in the system prompt shrinks to the synchronous form.
...
**Amendment 2026-09-12 (owner):** true parallel *execution* of those reads is deferred; the round-count benefit is delivered without it. In one response the model may request several read-only calls and the existing single-round loop runs all of a response's calls before the next round, so batching reads into one response (not one read per round) is where the saving lands. A prototype that made PenFileTools reads nonisolated (off the file actor) and batched them through a task group in the @MainActor ShepherdModel loop compiled the reads but hit a Swift 6 region-based-isolation-checker limitation running the `any ShepherdToolSource` existential concurrently; a clean version needs a dedicated concurrent-read method on the tool source (protocol + AppToolRouter + test fakes), not an inline task group and not `@unchecked Sendable` (AGENT.md). Parallel read I/O remains a future latency optimization; the paragraph above describes the intended end state.
