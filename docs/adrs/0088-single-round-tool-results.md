...
**Amendment 2026-09-12 (owner):** implemented. Read-only calls (`pen_read_file`, `pen_search`, `pen_glob`, `pen_list_files`) now execute concurrently. `PenFileTools` reads are `nonisolated`, so they run off the file actor; `ShepherdToolSource` gained an `invokeConcurrently` requirement with a sequential default and a concurrent override on the `@MainActor`, implicitly-Sendable `AppToolRouter` (a task group over its own `authorizeAndInvoke`); and `ShepherdModel` batches a run of consecutive read-only calls in one response, applying results in call order. Writes, edits and commands stay sequential with their approvals. An inline task group in `ShepherdModel` was avoided because capturing the `any ShepherdToolSource` existential tripped the Swift 6 region-isolation checker; routing the concurrency through the concrete router sidesteps that with no `@unchecked Sendable` (AGENT.md).
