...
The default provider deadline is five seconds, model tools have 120 seconds, and post-persist observers have 30 seconds. The host selects a bounded 630-second execution budget only for its active bundled Herder command supervisor: the command owns a separate maximum 600-second deadline and needs time to return its timeout receipt. Ordinary command timeout must not race the runtime watchdog and quarantine every Herder tool. Providers and model arguments cannot select this host policy. Authorization still precedes execution, and cancellation, revocation and the outer watchdog remain enforced. Limits also include 32 active extensions, 32 contributions per extension, 32 active turn snapshots, 16 in-flight calls per registration, 128 effective model tools, 64 KiB arguments and 256 KiB tool/service results. The API reference records the remaining payload limits. These are ceilings, not permission to perform arbitrary work.
