...
- Preserve ad-hoc development signing. `make build-signed` uses the ignored local Developer ID selection and accepts the build only after checking its publisher, team, hardened runtime, secure timestamp and entitlements. Official packaging, clean-tag validation and owner-approved publication retain their existing command-line release workflow.
