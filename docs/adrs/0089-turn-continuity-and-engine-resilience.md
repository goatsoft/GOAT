...
The engine client makes exactly one attempt per request. HTTP 408, 429 and 5xx are classified as connection failures but nothing retries them, and the stream uses the URL loading system's default 60 second idle timeout with no user-visible state while a large prompt prefills. The only loop guard is the file-repair tracker; a model that repeats an identical search or status call indefinitely is not caught. The reference harnesses keep failed turns with a synthetic marker, patch dangling tool calls with error results, retry transient errors with backoff before the first token, watch for stream stalls, and pause after three identical tool calls.
