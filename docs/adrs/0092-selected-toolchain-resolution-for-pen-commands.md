...
Prefer concrete toolchain paths over Apple shims. Preserve invocation names for driver aliases, while binding approval to the executable's filesystem identity and reporting its resolved path. Supply the selected toolchain PATH, macOS SDK and private module caches to child commands. For Xcode, admit the selected application bundle as a read-only runtime root, including versioned or custom bundle names: tools load sibling Frameworks and SharedFrameworks outside Contents/Developer. For Command Line Tools and other developer directories, retain the selected directory as the read root. Workspace/scratch writes, network grants, output limits, timeout and cancellation behavior remain unchanged.
