...
Settings displays a storage tree and an uninstall review with Partial uninstall selected by default. This keeps GOAT Home data and chats/attachments while selecting macOS preferences/window state for removal. Uninstall all checks every removal option; selecting Partial uninstall restores the default selection. Checkboxes select removal: checked items are removed and unchecked items are kept. Individual removal options remain editable, and retaining any category returns the mode to Partial uninstall. Entering the uninstall tab starts with the partial default. The separate local-data removal preview is removed; category choices live in uninstall, alongside direct preference reset ([ADR-0082](0082-direct-preference-reset.md)). Removing Pen metadata also selects chats and attachments; keeping chats preserves Pen metadata. External workspaces, referenced project files, model engines and remote memory are excluded. Only this app copy and an explicitly selected, signature-checked `goat` CLI can be removed.

The review is the final confirmation: an **Are you sure?** page lists what will be removed, what will be kept, the recovery folder and the requirement to quit normally. Its **Uninstall** button has no additional dialog. After that explicit confirmation, the app copies its complete signed application bundle into an owner-only temporary directory, verifies the copied signature, and launches the copied executable in its fixed `--goat-uninstall-helper` mode with a private typed request. The entry point dispatches this mode before creating AppModel or opening storage. There is no shell, privileged helper, launch agent, arbitrary command or network operation. The complete copy retains the Info.plist and sealed resources bound by Developer ID signing; a naked executable copy fails macOS signature validation even when an ad-hoc fixture succeeds. The helper validates its exact executable and bundle paths inside the private job; the JUDAS source check allows only this reviewed local process-launch site.
