...
Data management has two tabs: Reset preferences and Uninstall GOAT. Local-data choices are covered by the uninstall removal controls. Partial uninstall is the default and preserves GOAT Home data and chats/attachments while removing app preferences. Uninstall all checks every removal box. The uninstall review serves as the final **Are you sure?** confirmation, with a single **Uninstall** button, because it schedules app and data removal after shutdown.
