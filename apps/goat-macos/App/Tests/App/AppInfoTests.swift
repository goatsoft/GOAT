...
extension AppTests.App {
    @Suite struct AppInfoTests {
        @Test func hostedTestsUseAnExpandedIsolatedHome() throws {
            let environment = ProcessInfo.processInfo.environment
            #expect(environment["GOAT_TEST_MODE"] == "1")
            let home = try #require(environment["GOAT_HOME"])
            #expect(home.hasPrefix("/"))
            #expect(!home.contains("$("))
            #expect(URL(fileURLWithPath: home).lastPathComponent == "GOATTestsHome")
            #expect(home != FileManager.default.homeDirectoryForCurrentUser.appendingPathComponent(".goat").path)
        }
