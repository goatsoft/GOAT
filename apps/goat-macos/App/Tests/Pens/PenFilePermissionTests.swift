...
extension AppTests.Pens {
    @Suite struct PenFilePermissionTests {
        ...
        func nativeFileGrantsCoverCreateAndEditAtOnlyTheSelectedScope(wholePen: Bool) async throws {
            ...
            #expect(
                try String(contentsOf: fixture.root.appendingPathComponent("hello.txt"), encoding: .utf8) == "second\n")
        ...
        func rememberedWritesStillRejectEscapesAndChangedWorkspaceIdentity() async throws {
            ...
            #expect(
                await fixture.router.filePermissions.remember(
                    penID: fixture.pen, chatID: nil, workspaceIdentity: identity))
            let turn = UUID()
            ...
            _ = try await fixture.invoke(
                try #require(newRoutes["pen_write_file"]), arguments: #"{"path":"new.txt","content":"ok"}"#,
                choice: .allowOnce)
        ...
        func composerCanChoosePermissionsBeforeTheFirstFileCall() async throws {
            ...
            let identity = try PenFileTools(workspace: fixture.root).workspaceIdentity
            #expect(
                await permissions.select(.chat, penID: fixture.pen, chatID: fixture.chat, workspaceIdentity: identity))
            let turn = UUID()
            let routes = try await fixture.routes(chatID: fixture.chat, penID: fixture.pen, turnID: turn)
            _ = try await fixture.invoke(
        ...
        func composerAndPenChoicesReplaceExactlyTheDescribedScope() async throws {
            ...
            #expect(
                await permissions.select(.chat, penID: fixture.pen, chatID: fixture.chat, workspaceIdentity: identity))
            #expect(
                await permissions.select(.ask, penID: fixture.pen, chatID: fixture.chat, workspaceIdentity: identity))
            ...
            #expect(
                await permissions.select(.pen, penID: fixture.pen, chatID: fixture.chat, workspaceIdentity: identity))
