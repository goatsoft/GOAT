...
// and durable state. Fakes are the default; the live native coding case is explicitly opt-in.
...
extension AppTests.Shepherd {
    @Suite struct ShepherdModelTests {
        ...
        @Test func penBriefAndLegacyAgentGuideRemainDistinctFromWorkspaceFiles() async throws {
            ...
            let plan = try await worker.plan(
                snapshot: snapshot, model: ModelRef(id: "test-model", contextLength: 32_768), effort: .trot,
                tools: specs)
            let system = try #require(plan.request.turns.first?.text)
        ...
        @MainActor func liveNativeCodingUsesStructuredCallsAndVerifiesBytes(recoverMalformedResponse: Bool) async throws
        {
            ...
            while shepherd.hasActiveTurn && ContinuousClock.now < deadline {
                if let request = mcp.pendingPermission {
                    ...
                    if request.tool == "pen_run_command" {
                        let preview =
                            try JSONSerialization.jsonObject(with: Data(request.arguments.utf8)) as? [String: Any]
            ...
            print(
                ...
                "title", session.title)
            for message in session.messages where message.error != nil {
                print("LIVE_NATIVE_ERROR", message.error ?? "")
            }
            #expect(approvals >= 1)
            if !recoverMalformedResponse {
                ...
                #expect(
                    events.contains {
                        $0.tool == "pen_run_command" && $0.result?.contains("GOAT_COMMAND_VERIFIED") == true
                            && !$0.isError
                    })
            }
            ...
            let denied = ShepherdPromptSnapshot(
                ...
                messages: [
                    .init(
                        ...
                        toolEvents: [
                            .init(
                                id: "denied", requestName: "pen_write_file", arguments: "{}", result: "Denied",
                                isError: true,
                                denied: true)
                        ])
                ])
        ...
        @Test @MainActor func samplingRejectionRetriesOnceAndPublishesActualParameters() async throws {
            let engine = FakeEngine(
                ...
                failingRequest: 1,
                failingError: EngineError.httpDetail(400, "Unsupported parameter: top_k", retryAfter: nil))
        ...
        func compactionProgressIsVisibleOnlyWhileTheOperationOwnsIt(fails: Bool) async {
            ...
            let engine = FakeEngine(
                script: [
                    [.token("Shader constraints retained."), .done(GenStats(ttft: nil, tokens: 4, duration: 0.01))]
                ],
