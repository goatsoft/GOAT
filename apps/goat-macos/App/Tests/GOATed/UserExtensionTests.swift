...
extension AppTests.GOATed {
    @Suite struct UserExtensionTests {
        ...
        @MainActor @Test func userPackageInstallEnableDisableRestoreAndRemoveAreScoped() async throws {
            ...
            #expect(
                !FileManager.default.fileExists(atPath: root.appendingPathComponent(package.manifest.id + ".json").path)
            )
        }
        ...
        @MainActor @Test func userPackageReviewPinsBytesAndExportsWithoutEnabling() async throws {
            ...
            let manager = UserExtensionManager(
                runtime: ExtensionRuntime(), root: root.appendingPathComponent("installed"))
