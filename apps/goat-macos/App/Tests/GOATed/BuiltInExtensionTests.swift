...
extension AppTests.GOATed {
    @Suite struct BuiltInExtensionTests {
        ...
        @Test func herderCommandTimeoutUsesConfigurationUnlessExplicitlyOverridden() async throws {
            ...
            let explicit = try await runner.prepare(
                argumentsJSON: #"{"command":"/bin/pwd","args":[],"timeout_seconds":45}"#)
            let first = try #require(
                JSONSerialization.jsonObject(with: Data(command.previewJSON.utf8)) as? [String: Any])
            let second = try #require(
                JSONSerialization.jsonObject(with: Data(explicit.previewJSON.utf8)) as? [String: Any])
