...
The background, CLI icon and Applications folder icon were created with the built-in ImageGen tool from the owner's Midnight base and icon references. The final background was rebuilt in one ImageGen pass from the original Midnight base to avoid accumulated texture loss from successive edits. They replace the first circular-layout exports. Source masters retain the generated artwork and transparency; packaging renders the final background locally. These pictures are Brand Assets under [LICENSE-ART.md](../../../../LICENSE-ART.md). The authoring [prompts](PROMPTS.md) are retained for reference; measured export geometry governs Finder placement.
...
- `goat-applications-icon.png`: blue folder with the standard white Applications tools symbol. The destination carries no GOAT branding.
...
| --- | --- | --- | --- |
| GOAT.app | (220, 183) | (220, 183) | 96 |
| Applications | (499, 183) | (499, 183) | 96 |
| CLI Tools | (303, 247) | (303, 271) | 48 |
...
The CLI recess sits below and to the right of the app recess with a visible gap. Finder retains dark native filename labels over picture backgrounds. Lightly tinted see-through pale-blue pills back the GOAT, CLI Tools and Applications labels for contrast. The installation sentence sits at the bottom centre, below the support-folder row.

Applications is a regular Finder alias to `/Applications`, with its own custom icon. Packaging verifies the resolved destination and preserves the icon resource through conversion and read-only remount. It never sets an icon on the actual `/Applications` folder. Licence retains the existing notice folder and uses a native text-document icon.
