...
Precise icon correction and background extraction. Image1 is the folder icon edit target. Image2 is the exact CLASSIC macOS Applications glyph reference. Replace the modern App Store rounded-stick A on the folder with the classic Applications A made of three distinct drawing tools: paintbrush, pencil and ruler, closely matching the white glyph in image2. Keep it white and centred at the same size. This must NOT be the modern App Store logo made of three rounded popsicle sticks. Preserve the blue folder shape and shading. Also REMOVE ALL grey checkerboard pixels outside the folder and set that entire exterior to actual PNG alpha=0. The checkerboard is currently baked into the pixels; do not repaint it. Output a true transparent PNG cutout with clean antialiased folder edges. No GOAT logo, no words, no background, no shortcut arrow. Only the blue folder with the standard white classic Applications tools symbol.

## Applications icon transparency extraction

Use case: background-extraction. Return this exact blue Applications folder icon as a transparent PNG cutout. Remove ALL grey checkerboard outside the folder; it is currently baked into the image and must become actual alpha=0. Preserve every pixel inside the blue folder, including the exact white crossed paintbrush, pencil and ruler symbol. Do not redesign the icon. No background, no white matte, no black matte and no drawn checkerboard. True PNG transparency outside the folder, with clean antialiased outer edges. This is exclusively an alpha extraction, with no content edits.
